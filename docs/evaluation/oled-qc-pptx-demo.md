...
Agent recommendation은 `needs_changes`다. 전체 demo에서는 이 판단을 숨기지 않고, 관리자가
“동일 파일 완료평가 실패 경로를 검증하는 offline demo”라는 제한된 이유로 등록을 명시적으로
승인한다. 이 승인 예제는 실제 과제의 등록 승인 근거가 아니다.
...
Agent recommendation은 `not_accept`다. demo의 관리자도 동일 hash와 수행증거 부재를 이유로
`not_accept`를 명시적으로 확정한다.
