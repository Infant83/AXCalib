...
## 3. 2026-07-22 Library/CLI/Resource API local Alpha checkpoint

- 현재 Phase: P7 Interfaces
- 완료 checkpoint: WP-01.R1.2 + WP-06.I1 + WP-06.I2a/I2b principal-bound project·education API
- 다음 실행 slice: WP-06.I2c project read/decision replay (`ready`)
- 현재 Gate: G4 Interfaces (`in_progress`; CLI/batch/resource API local contract evidence 확보)
...
WP-01.R1.2, Alpha CLI, WP-06.I1 runtime과 WP-06.I2a/I2b project·education resource command는
완료됐다. 다음은 project decision 응답 유실을 복구하기 위한 G4 계약 slice다.

1. 먼저 `git status`와 `PROJECT_STATE.md`의 WP-06.I2c Exit Evidence를 확인한다.
2. project owner/admin의 authorized GET을 추가하되 dossier/local URI와 허용되지 않은 audit 원문을 숨긴다.
3. registration/completion decision에 principal+idempotency key 기반 semantic replay를 추가한다.
4. 같은 key의 다른 command/context는 충돌시키고 commit 뒤 응답 유실 재시도를 회귀한다.
5. 실제 OIDC·계정·배포 없이 synthetic in-process contract로 시작한다.
