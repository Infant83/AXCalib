...
## 3. 2026-07-22 Library/CLI/API local Alpha checkpoint

- 현재 Phase: P7 Interfaces
- 완료 checkpoint: WP-01.R1.2 Library MVP/Alpha + WP-06.I1 runtime API Alpha
- 다음 실행 slice: WP-06.I2 principal-bound command contract (`ready`)
- 현재 Gate: G4 Interfaces (`in_progress`; CLI/batch/API local Alpha evidence 확보)
- 최근 검증: 110 lightweight tests, 10 eval groups, validation 0/0, Ruff, Pyright 0/0
...
- generic API에서 actor/admin decision을 전달하거나 local Alpha를 운영 OIDC/RBAC/server로 간주하는 것
...
`prep.ps1 docling`으로 분리했다. live model은 별도 opt-in이며 승인된 비식별 자료만 사용한다.

## 6. 다음 작업을 시작하는 방법

WP-01.R1.2, Alpha CLI와 WP-06.I1 runtime API는 완료됐다. 다음은 사람 권한 command를 안전하게
노출하기 위한 G4 계약 slice다.

1. 먼저 `git status`와 `PROJECT_STATE.md`의 WP-06.I2 Exit Evidence를 확인한다.
2. project/education command의 request actor를 신뢰하지 않고 verified principal에 bind한다.
3. organization/project/enrollment scope mismatch를 mutation 전에 거부한다.
4. remote request가 arbitrary local path를 전달하지 않도록 upload/staging/content-hash 경계를 만든다.
5. 구현한 resource만 implemented OpenAPI에 추가하고 target artifact 전체를 완료로 승격하지 않는다.
6. 실제 OIDC·계정·배포 없이 synthetic in-process contract로 시작한다.

Library 중단 원인과 Alpha 리뷰는
[`wp01-r1-2-library-mvp-alpha-report.md`](evaluation/wp01-r1-2-library-mvp-alpha-report.md), API 권한
리뷰는 [`wp06-i1-minimal-api-parity-report.md`](evaluation/wp06-i1-minimal-api-parity-report.md)에 있다.
