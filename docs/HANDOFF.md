...
## 3. 2026-07-22 Library MVP/Alpha checkpoint

- 현재 Phase: P7 Interfaces
- 완료 checkpoint: WP-01.R1.2 Library MVP/Alpha (`verified_local_library_alpha`)
- 다음 실행 slice: WP-06.I1 minimal API parity (`ready`)
- 현재 Gate: G4 Interfaces (`in_progress`; CLI/batch Alpha evidence 확보)
- 최근 검증: 103 lightweight tests, 10 eval groups, validation 0/0, Ruff, Pyright 0/0
...
WP-01.R1.2와 Alpha CLI는 완료됐다. 다음은 G4의 가장 작은 HTTP delivery slice다.

1. 먼저 `git status`와 `PROJECT_STATE.md`의 WP-06.I1 Exit Evidence를 확인한다.
2. FastAPI는 optional `api` extra와 `src/axcalib/api` adapter 안에만 둔다.
3. actual handler가 `AXCalib.registry`와 `LocalPipelineExecutor`를 직접 호출하게 한다.
4. committed OpenAPI 3.1 artifact와 catalog/run/status/cancel response parity를 검증한다.
5. bearer auth와 role boundary는 contract-only로 만들고 운영 계정·배포는 생성하지 않는다.
6. in-process test 뒤 문서/diagram/ledger를 같은 change set에서 갱신한다.

중단 원인과 Alpha 코드리뷰는
[`wp01-r1-2-library-mvp-alpha-report.md`](evaluation/wp01-r1-2-library-mvp-alpha-report.md)에 있다.
