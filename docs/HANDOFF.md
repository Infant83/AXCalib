...
| Case | project ID로 최신 Dossier와 두 Gate 결과를 다시 읽는 live handle |

## 3. 2026-07-24 Library usability and Qwen verification CLI checkpoint

- 현재 Phase: P5 Evaluation
- Active Slice: WP-05.Q3 exact on-prem Qwen execution (`ready`)
- 완료 checkpoint: WP-01.R1.2 + WP-06.I1~I4 local reference + WP-00.Q1 Case/readability audit +
  WP-06.I5a Qwen verification CLI
- 완료 품질 slice: GOAL alignment, public API/script usability와 EX-01~14 example self-check
- 현재 품질 준비: WP-03.Q2a Owner 승인 Markdown + policy YAML + adjudicated gold JSONL + hash
  manifest 입력계약과 hidden test split runner 완료
- 운영 dependency: approved remote identity/assignment/immutable upload boundary (`blocked_policy`)
- 현재 Gate: G3 Intelligence (`reference_verified_exact_onprem_ready_quality_pending`);
  G4 local CLI/batch/resource API/local Worker checkpoint 유지
- 최근 전체 검증 수치는 `PROJECT_STATE.md` 7절을 기준으로 확인한다.
- 문서 checkpoint: WP-00.D2 GitHub Wiki live/automatic publication, GitLab portable export contract

현재 가능한 것:

- 제공 PPTX를 dossier로 등록하고 revision을 고정한다.
- 등록 직후 받은 `Case`에서 현재 단계·다음 action과 두 Gate 요약을 object/JSON/Markdown으로 읽는다.
...
- 사내에서는 `axcalib verify qwen`으로 exact model identity, structured text와 synthetic vision을
  secret-free JSON report로 확인한다.
...
- optional identity adapter에서 synthetic signed access token의 signature, issuer/audience/type/time,
  issuer-bound key와 exact role/scope/org mapping을 검사한다. invalid는 401, key source/config 장애는
  503으로 구분한다.
...
- generic API에서 actor/admin decision을 전달하거나 local signed identity reference를 운영
  OIDC/RBAC/upload/server로 간주하는 것
...
`prep.ps1 docling`으로 분리했다. integration은 core/eval/ops shard로 더 나눴으므로 중단되면 해당
묶음만 다시 실행한다. live model은 별도 opt-in이며 승인된 비식별 자료만 사용한다.

## 6. 다음 작업을 시작하는 방법

WP-01.R1.2, Alpha CLI, WP-06.I1~I4 local reference, WP-00.Q1 Library 사용성 감사와
WP-06.I5a Qwen verification CLI까지 완료됐다. 다음은 dependency에 따라 진행한다.

1. 사내 endpoint에서 exact Qwen capability와 테스트 PPTX registration report를 만든다. completion은
   승인된 completion template/evidence가 있을 때만 수행한다.
2. Evaluation Owner가 공식 rubric, threshold와 숨겨 둔 test split의 adjudicated gold fixture를
   승인하면 WP-03.Q2b report benchmark를 진행한다.
3. 승인된 corpus/label 뒤 WP-04 embedding/Qdrant/rerank benchmark와 P6 calibration을 진행한다.
4. Product/Security Owner가 decision packet의 OIDC issuer/claim mapping, organization·assignment source와
   revocation 책임을 승인한다.
5. upload/staging owner가 immutable object version, ACL, malware scan, media/size/hash와 retention을 확정한다.
6. 승인 전에는 실제 계정·업로드·운영 server를 만들지 않는다.
7. distributed worker가 필요하면 broker/database, heartbeat, dead-letter, metrics와 payload encryption/
   retention을 ADR로 분리한다.
8. 승인된 dependency 뒤에만 Web, socket E2E와 penetration/load test를 Gate evidence로 추가한다.
...
identity와 운영 결정 경계는
[`wp06-i4-identity-jwks-reference-report.md`](evaluation/wp06-i4-identity-jwks-reference-report.md)와
[`identity-upload-decision-packet.md`](security/identity-upload-decision-packet.md)에 있다.
Library 사용성·GOAL 정렬과 예제 catalog 결과는
[`wp00-q1-library-standardization-report.md`](evaluation/wp00-q1-library-standardization-report.md)에
있다.
Evaluation Owner 입력법, Docling 자원 차단과 SkillBoss/Qwen 진단은
[`wp03-q2a-evaluation-owner-input-contract-report.md`](evaluation/wp03-q2a-evaluation-owner-input-contract-report.md)에
있다. 사내 실행 interface와 runbook 리뷰는
[`wp06-i5a-qwen-verification-cli-report.md`](evaluation/wp06-i5a-qwen-verification-cli-report.md)에 있다.
