...
- 현재 실행 slice: WP-01.R1 Transaction Journal / Reconciliation (`in_progress`)
- 현재 Gate: G2 offline reference를 통과했지만 운영 hardening은 남아 있음
- 최근 검증: 88 tests, 9 eval groups, validation 0 errors/0 warnings, Ruff/Pyright 통과
...
WP-01.R1.1에서는 project dossier/audit recovery와 HITL report/outbox prerequisite를 구현했다. 다음
R1.2의 목표는 아직 journal 앞에서 생성되는 side effect와 education 파일을 같은 복구 경계로 넓히는 것이다.

1. EducationEnrollment, education audit와 program completion outbox의 write boundary를 열거한다.
2. report/outbox를 journal prepare 이후의 idempotent producer 단계로 이동한다.
3. stale lock의 소유자·나이 판정과 강제 해제 대신 quarantine하는 정책을 구현한다.
4. orphan temp/journal retention과 compaction 기준을 정한다.
5. 각 새 boundary에 synthetic crash를 주입하고 반복 reconcile의 무중복을 검증한다.
