...
## 3. 2026-07-24 Library usability standardization checkpoint

- 현재 Phase: P7 Interfaces
- 완료 checkpoint: WP-01.R1.2 + WP-06.I1~I4 local reference + WP-00.Q1 Case/readability audit
- 완료 품질 slice: GOAL alignment, public API/script usability와 EX-01~13 example self-check
...
WP-01.R1.2, Alpha CLI, WP-06.I1~I4 local reference와 WP-00.Q1 Library 사용성 감사까지 완료됐다.
다음은 Owner가 제공하는 품질 또는 운영 dependency에 따라 두 갈래 중 하나를 선택한다.

1. Evaluation Owner가 공식 rubric, threshold와 숨겨 둔 test split의 adjudicated gold fixture를
   승인하면 WP-03.Q2b report benchmark를 진행한다.
2. Product/Security Owner가 decision packet의 OIDC issuer/claim mapping, organization·assignment source와
   revocation 책임을 승인한다.
3. upload/staging owner가 immutable object version, ACL, malware scan, media/size/hash와 retention을 확정한다.
4. 승인 전에는 실제 계정·업로드·운영 server를 만들지 않는다.
5. distributed worker가 필요하면 broker/database, heartbeat, dead-letter, metrics와 payload encryption/
   retention을 ADR로 분리한다.
6. 승인된 dependency 뒤에만 socket E2E와 penetration/load test를 Gate evidence로 추가한다.
