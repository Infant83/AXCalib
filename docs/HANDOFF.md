...
## 3. 2026-07-22 Library/CLI/Project API local Alpha checkpoint

- 현재 Phase: P7 Interfaces
- 완료 checkpoint: WP-01.R1.2 + WP-06.I1 + WP-06.I2a principal-bound project API
- 다음 실행 slice: WP-06.I2b education principal binding (`ready`)
- 현재 Gate: G4 Interfaces (`in_progress`; CLI/batch/project API local contract evidence 확보)
...
WP-01.R1.2, Alpha CLI, WP-06.I1 runtime과 WP-06.I2a project command는 완료됐다. 다음은 교육
프로그램의 사람 역할을 안전하게 노출하기 위한 G4 계약 slice다.

1. 먼저 `git status`와 `PROJECT_STATE.md`의 WP-06.I2b Exit Evidence를 확인한다.
2. learner/mentor/instructor/administrator principal을 enrollment/program/project context에 bind한다.
3. program version·learner·mentor·organization scope mismatch를 mutation 전에 거부한다.
4. project API의 role을 교육 authority로 자동 승격하지 말고 typed command별 matrix를 만든다.
