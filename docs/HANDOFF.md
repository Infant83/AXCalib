...
## 3. 2026-07-22 Library/CLI/Resource API Read-Replay local Alpha checkpoint

- 현재 Phase: P7 Interfaces
- 완료 checkpoint: WP-01.R1.2 + WP-06.I1 + WP-06.I2a/I2b/I2c project·education API/read/replay
- 다음 실행 slice: WP-06.I3 durable local 202 worker contract (`ready`)
...
완료됐다. 다음은 오래 걸리는 작업을 request process와 분리하는 G4 worker 계약 slice다.

1. 먼저 `git status`와 `PROJECT_STATE.md`의 WP-06.I3 Exit Evidence를 확인한다.
2. parse/evaluation 대상 request는 inline 실행 대신 202와 stable run reference를 반환하게 한다.
3. local durable queue/claim, worker restart와 retryable-only resume를 기존 checkpoint/idempotency에 연결한다.
4. poll/cancel의 owner/admin/scope와 result hash/URI redaction을 회귀한다.
5. 실제 broker·OIDC·계정·배포 없이 synthetic in-process worker contract로 시작한다.
