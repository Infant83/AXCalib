...
| D-040 | 2026-07-23 | Accepted | 사용자 Wiki는 main `wiki/`를 단일 원본으로 두고 PROJECT_STATE 개발원장을 자동 mirror한다. GitHub/GitLab Wiki는 manifest 기반 target이며 sidebar 파일명만 변환한다. remote URL은 환경변수로 주입하고 publish는 opt-in·명시적 push로 제한한다. |
| D-041 | 2026-07-24 | Accepted | 운영 IdP 값을 임의로 만들지 않고 optional `identity` extra에 provider-neutral RFC 9068 OIDC/JWKS local reference를 둔다. fixed asymmetric algorithm allowlist, exact issuer/audience/type/time, issuer-bound key snapshot과 exact role/scope mapping을 검증하며 invalid token은 401, key provider/config 장애는 503으로 fail closed한다. remote discovery/rotation/revocation과 실제 assignment는 Owner 승인 뒤 별도 구현한다. |
| D-042 | 2026-07-24 | Accepted | `register_case`는 최신 dossier를 project_id로 다시 읽는 live `Case`를 반환한다. status/summary는 Agent 원본과 사람 결정·criterion 보정을 분리하며 report root/identity/artifact hash와 active/archive transaction SHA-256을 검증한다. 기본 출력은 local URI·storage path·사람 사유를 redaction하고 verbose local view는 remote API 권한을 대신하지 않는다. raw dossier는 `case.dossier`, initial snapshot 호환은 `create_project`로 둔다. |
| D-043 | 2026-07-24 | Accepted | 공식 semantic benchmark 입력은 Owner 승인 Markdown, published review-policy YAML, reviewer vote와 adjudication을 가진 project-stage JSONL, hash-bound manifest로 분리한다. 구현자는 threshold와 gold 정답을 만들지 않으며 approved status·숨겨 둔 test split의 두 Gate·두 reviewer·adjudication·hash가 모두 없으면 공식 quality pass/fail을 반환하지 않는다. |
