...
timezone: Asia/Seoul
status: g4_qwen_verification_cli_operational_policy_pending
---
...
2026-07-24 WP-06.I5a에는 canonical environment 검증과 exact Qwen capability 실행을 한 Library
service로 모으고 script와 `axcalib verify qwen`이 같은 report/exit 의미를 사용하도록 연결했다.
...
Library Alpha의 local executor는 request SHA-256, replay-relevant context와 result SHA-256을 run
checkpoint에 고정하고 같은 run ID를 filesystem lease로 직렬화한다. terminal/cancelled 결과는
재실행하지 않고 retryable failure만 재시도한다. cancel은 process kill이나 이미 commit된 domain
mutation rollback이 아니라 cooperative marker다. education/project reconcile과 workspace maintenance도
같은 allowlisted registry에 있으며 상세 결정은 ADR-021을 따른다.
...
`PROJECT_STATE.md`는 architecture diagram의 복제본이 아니라 실행 관점의 단일 원장이다. P/WP/G
dependency Gantt, Active Slice, Gate 판정, 실제 검증과 append-only 작업 이력을 관리하며, 상세
...
```mermaid
flowchart LR
    P["EducationProgram@version\nimmutable + SHA-256"] --> E["EducationEnrollment\nlearner goals + revision"]
    E --> M["MilestoneProgress\nmanual / score / project"]
    M --> D["ProjectDossier\nregistration + completion gates"]
    D -->|"stored completion_accepted"| M
    M --> READY["all required complete"]
    READY --> N["program completion notification"]
    N --> H{{"administrator HITL"}}
    H -->|approve| DONE["enrollment completed"]
    H -->|return| REOPEN["selected milestone needs_action"]
```

설계 불변조건:

- program은 `program_id@version`별 immutable이고 enrollment는 exact hash에 pin한다.
- level/milestone/prerequisite는 DAG지만 앞선 milestone만 참조한다.
- alpha requirement union은 `manual_confirmation`, `score_at_least`, `project_status`다.
- milestone pipeline은 registry allowlist에 등록된 ID/version만 허용한다.
- project milestone은 program/version/enrollment/milestone/learner context가 모두 일치해야 한다.
- project 상태는 dossier repository에서 읽으며 API 요청 값으로 완료를 주장할 수 없다.
- 모든 필수 milestone 충족은 관리자 대기 상태만 만들고 credential을 자동 발급하지 않는다.
- 새 program version은 신규 enrollment에만 적용한다. migration/rollout은 별도 decision이다.

공개 최소 진입점은 `publish_program(...)`, `run_education(...)` / `arun_education(...)`이며 typed
command가 `education-program-runtime@v1alpha1`을 호출한다. 향후 CLI/API/Web도 이 service를 직접
호출하고 browser에서 progression을 재계산하지 않는다. 상세 결정은 ADR-017을 따른다.
...
- 확장자: .axc.yaml
- schema ID: axcalib.dossier/v1alpha2
...
on-prem 전문 설정은 `OPENAI_STRUCTURED_OUTPUT_MODE=json_schema|json_object`와
`OPENAI_MAX_OUTPUT_TOKENS`를 추가로 받을 수 있다. dialect와 생성한도는 명시적으로 기록하며,
provider 오류 뒤 다른 dialect/model로 자동 재시도하지 않는다. 어느 mode에서도 최종 text는
Pydantic schema와 evidence locator guard를 다시 통과해야 한다.

`json_object`는 provider가 schema를 직접 강제하지 않으므로 gateway가 literal `JSON`, contract
name과 canonical compact schema를 system instructions에 포함한다. proxy가 upstream JSON error를
top-level detail string에 감싸면 status/type/code/param처럼 allowlisted identifier만 추출하며 server
message나 evidence는 예외에 넣지 않는다.
...
probe는 `provider_proxy`와 `deployment` scope를 분리한다. deployment scope는 requested model,
endpoint가 실제 보고한 response model, expected checkpoint가 명시적으로 일치해야 한다. provider
alias는 별도 공급자 증명 없이 exact checkpoint로 확장하지 않는다. report에는 raw prompt/image/output와
`reasoning_content`를 저장하지 않고 hash, latency, safe failure kind만 남긴다.

공통 `MultimodalCapabilityProbe`는 Qwen 외 승인된 route에도 같은 synthetic text/vision을 적용한다.
기본 `provider_proxy` scope는 model ID가 일치해도 deployment-ready가 될 수 없다. 미지원 기능은
fallback하지 않고 명확히 실패하며, invalid structured output을 자동 repair해 성공으로 바꾸지 않는다.

Qwen 전용 environment composition은 `probe_qwen35_from_env()` 한 곳에서 canonical `OPENAI_*`,
Qwen3.5 route, exact checkpoint와 vision 포함 여부를 검증한다. working script와
`axcalib verify qwen`은 이 service의 파일/argument/exit-code adapter이며 provider나 판정 규칙을
복제하지 않는다.
...
- 사용자 계획상 첫 exact 검증 ID는 `Qwen3.5-397B-A17B`; SkillBoss의 `qwen3.5-plus`는 proxy로만 취급
...
~~~json
{"item_id":"item-001","pipeline_id":"dossier.freeze","pipeline_version":"v1alpha1","payload":{"project_id":"...","expected_revision":3},"idempotency_key":"item-001-r3"}
~~~

JSONL은 한 줄에 한 item이며 `batch_id`는 파일 stem 또는 CLI `--batch-id`에서 얻는다. manifest
SHA-256을 먼저 고정하고 10 MiB/10,000 item을 초과하면 실행하지 않는다.
...
batch는 checkpoint와 per-item result를 기록한다. resume은 retryable failure와 미실행 항목만 다시
처리하고 succeeded/waiting/blocked/stale/terminal/cancelled 항목을 중복 실행하지 않는다.
...
Docling은 lazy optional adapter이며 기본 lightweight test에는 포함하지 않는다. `prep.ps1 docling`을
별도 실행해 구조 계약을 확인한다. image-only fixture의 기본 quickstart는 OOXML 안전검사,
embedded-image provenance와 원본 hash에 고정된 reviewed sidecar를 사용하며, sidecar가 없으면 시각
의미를 만들지 않는다.

### 14.1.1 현재 제한형 actual-PPT reference

현재 adapter는 PowerPoint·LibreOffice·외부 endpoint에 의존하지 않고 OOXML package 안의 visual
pixel을 그대로 추출한다. 지원 계약은 다음처럼 의도적으로 작다.

| 입력 형태 | 처리 |
|---|---|
| uncropped, unrotated, near-full-slide embedded PNG 1개 | PNG bytes를 content-addressed slide artifact로 추출 |
| object가 없는 true blank slide | 다른 visual slide와 같은 크기의 deterministic white PNG 생성 |
| text box, chart, group, multiple picture, crop/rotation, external image, macro | `SlideRenderError`로 fail-closed |

`SlideRenderManifest`는 source SHA-256, renderer ID, slide 수, visual/blank 구분, PNG 크기·byte 수·
SHA-256과 상대 artifact URI를 기록한다. 절대 output path와 timestamp를 넣지 않아 동일 입력의 구조적
manifest hash를 반복 비교할 수 있다.

`EvidenceGoldDataset`은 원문 요약을 복제하지 않고 reviewed summary SHA-256, slide locator와
tag만 보존한다. loader는 원본 PPTX·sidecar hash와 13개 annotation 전체를 비교하며 drift 시
실패한다. `EvidenceQualityReport`는 render coverage, locator recall, reference-field coverage,
criterion locator 해소, insufficient-evidence와 unsupported assertion을 분리한다. 이 계약은 visual
semantic correctness나 공식 평가 품질을 측정하지 않는다.
...
6. synthetic PPTX 3종의 Docling/slide extraction spike
   - supplied image-only fixture의 제한형 16-slide render/gold regression은 완료
   - 합성 slide와 실제 복수 template/OCR/VLM coverage는 남음
