...
| R-021 | local actor ID를 실제 관리자 인증으로 오해 | demo 결과가 운영 승인처럼 사용됨 | demo actor label, injected verifier/grant, generic actor 거부, project/education audit actor의 verified principal binding, local signed OIDC reference | Local JWT contract mitigated; approved issuer/remote JWKS/revocation open |
...
| R-037 | Docling과 정적검사 동시 실행 또는 긴 test process가 저메모리·시간제한 개발환경을 고갈 | 회귀 중단·불완전한 품질 주장 | Docling lazy import와 별도 `prep.ps1 docling`, lightweight parser, unit/integration-core/eval/ops/contract shard, 순차 검증과 512 MiB Pyright cap | Locally mitigated; CI resource sizing open |
...
| R-039 | local Alpha API를 운영 server로 사용 | tenant 간 노출, 과대 request, path/credential 오용 | in-process 라벨, structural URI redaction, project size/hash guard, typed 1 MiB queued payload, local OIDC negative contract와 운영 NO-GO | Open until approved remote identity, immutable upload, proxy limits and distributed worker |
...
| R-043 | mentor/instructor resource scope가 실제 교육 배정 원장과 다름 | 다른 사람이 확인·점수를 기록하거나 tenant가 섞임 | exact enrollment/program scope, organization, role와 audit actor, external role/scope exact mapping을 검사하고 기본 verifier는 거부 | Open until approved assignment source and revocation tests |
...
| R-045 | GitHub/GitLab Wiki와 main 문서 drift, transient local write 또는 잘못된 remote push | 사용자가 오래된 매뉴얼을 보거나 사내 정보·자격증명이 유출됨 | main `wiki/` 단일 원본, ledger mirror, manifest/link/parity 검증, atomic replace bounded retry, env-only remote, dirty/origin guard와 opt-in CI; direct edit·force push 금지 | Local export mitigated; actual GitLab publication and credential review open |
| R-046 | JWT algorithm/token confusion, token-controlled key source 또는 stale JWKS | 위조·다른 용도의 token이 principal로 승격되거나 key 장애 중 잘못 승인 | RFC 9068 `at+jwt`, asymmetric policy allowlist, exact issuer/audience/time/JTI, `jku/x5u/x5c` 거부, issuer-bound unique-kid snapshot, invalid 401/provider 503, raw token 비저장 | Local signed contract mitigated; approved remote discovery/cache/rotation/revocation and security test open |
| R-047 | local `Case` verbose projection을 API/Web safe view로 오용 | actor/rationale, criterion excerpt 또는 review context가 권한 밖에 노출 | 기본 projection은 URI/path/사람 상세를 redaction하고 locator를 artifact ID로 치환; report는 committed journal hash와 identity를 재검증; Wiki/ADR에 remote property authorization 경계 명시 | Local Library mitigated; principal-bound Case API projection open |
