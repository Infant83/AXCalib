...
| R-016 | TOML/API로 필수 HITL·알림을 우회 | 무권한 인증 상태 전이 | protected config, domain guard, generic authority field 거부와 principal-bound project decision endpoint | Mitigated for local project API; education/full API open |
...
| R-021 | local actor ID를 실제 관리자 인증으로 오해 | demo 결과가 운영 승인처럼 사용됨 | demo actor label, injected verifier/grant, generic actor 거부, project audit actor의 verified principal binding | Project API locally mitigated; approved OIDC/JWKS open |
...
| R-038 | Library registry 등록을 HTTP 공개 권한으로 오인 | 임의 local path 실행·actor 가장·사람 Gate 우회 | exact grant, generic authority 거부, 전용 project role/scope/org/revision command와 no-path staging | Mitigated for implemented routes; education/full authorization open |
...
| R-042 | 관리자 decision commit 뒤 HTTP 응답 유실 | client가 결과를 확인하지 못하거나 무작정 재시도 | expected revision으로 중복 mutation은 차단 | Open until semantic replay/idempotency record and authorized project GET |
