...
| R-016 | TOML/API로 필수 HITL·알림을 우회 | 무권한 인증 상태 전이 | protected config, domain guard와 generic API authority field 거부; principal-bound 전용 command는 미구현 | Mitigated for local generic API; dedicated endpoint open |
...
| R-021 | local actor ID를 실제 관리자 인증으로 오해 | demo 결과가 운영 승인처럼 사용됨 | demo actor label, API injected verifier/grant와 generic actor field 거부 | Local API mitigated; approved OIDC/RBAC open |
...
| R-038 | Library registry 등록을 HTTP 공개 권한으로 오인 | 임의 local path 실행·actor 가장·사람 Gate 우회 | 별도 exact `ApiPipelineGrant`, 빈 기본 grant, nested authority field 거부와 owner/scope contract test | Mitigated for generic local API; endpoint-specific authorization open |
| R-039 | local Alpha API를 운영 server로 사용 | tenant 간 노출, 과대 request, path/credential 오용 | in-process 라벨, filesystem URI redaction, 배포 금지 경계 | Open until OIDC/RBAC, upload boundary, limits and threat model |
