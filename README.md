...
principal-bound resource API/read-replay, WP-06.I3 durable local 202 Worker Alpha** 단계다. Python Library, local pipeline,
...
- Vector DB/Qdrant, 실제 embedding 품질, multi-model calibration, full product CLI/evaluation API,
  OIDC/JWKS·실제 교육 배정 원장·immutable upload·Web, report/outbox producer transaction과 distributed
...
- 사용자용 `wiki/` 단일 원본과 `PROJECT_STATE.md` 개발원장 mirror, GitHub/GitLab target export가
  ...
  push/render됐고 자동 publish variable도 활성화했다. 사내 GitLab runner/credential 설정은 남아 있다.
...
GitHub target은 2026-07-23 현재 실제 publication과 page/image render를 검증했다. GitLab target은
사내 개발서버의 runner, deploy credential과 Wiki remote가 준비된 뒤 같은 명령으로 검증한다.
...
실행한다. 긴 단일 pytest 대신 unit/integration/contract를 독립 process로 실행하며, 중단 뒤에는
`.\prep.ps1 test unit|integration|contract` 중 필요한 group만 재실행할 수 있다.
...
1. G4 다음 slice에서 approved OIDC/JWKS claim mapping, 실제 교육 배정 원장과 immutable upload
   boundary를 설계·검증한다.
2. Alpha CLI를 목표 `dossier/evaluate/cases/batch/report/verify` 명령 UX로 점진 확장한다.
3. report/outbox producer transaction과 database/broker worker heartbeat·dead-letter·distributed lease를
   별도 hardening한다.
