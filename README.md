...
단계다. Python Library, local pipeline, working script가 dossier·snapshot·평가초안·HITL
...
- Vector DB, multi-model calibration, durable outbox, CLI/API/Web은 아직 구현되지 않았다.
...
1. Product/Evaluation Owner가 실제 rubric, context→profile mapping과 합격선을 승인한다.
2. 실제 제안/완료 template의 field·locator와 slide-render/VLM 계약을 fixture로 고정한다.
3. labeled retrieval/model golden dataset을 늘리고 Qdrant·embedding·multi-model을 별도 평가한다.
4. dossier JSON Schema, idempotency, stale result, report integrity와 durable outbox를 보강한다.
