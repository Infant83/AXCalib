...
reference**, **WP-01.R1 project transaction recovery 진행 중** 단계다. Python Library, local pipeline, working
script가 dossier·snapshot·평가초안·HITL
...
- project create/update는 hash-chained append-only journal로 dossier/audit를 복구한다. HITL report와
  recorded outbox가 바뀌면 차단하고, reconcile은 알림을 재전송하지 않는다. education transaction,
  report/outbox producer와 stale-lock cleanup은 아직 남았다.
- Vector DB/Qdrant, 실제 embedding 품질, multi-model calibration, CLI/API/Web은 아직 구현되지 않았다.
...
기본 `prep.ps1 test|eval`은 외부 endpoint를 호출하지 않는다.
...
1. `WP-01.R1.2`에서 education/report-outbox producer transaction과 stale lock/orphan recovery를 보강한다.
2. exact on-prem `Qwen3.5-397B-A17B`에서 registration/completion과 serving fingerprint를 검증한다.
3. Product/Evaluation Owner가 실제 rubric, context→profile mapping과 합격선을 승인한다.
4. 다른 실제 제안/완료 template의 field·locator와 일반 renderer/OCR/VLM 계약을 fixture로 고정한다.
5. labeled retrieval/model golden dataset을 늘리고 Qdrant·embedding·multi-model을 별도 평가한다.
6. program version rollout/retire와 enrollment migration 정책은 과정 Owner 승인 뒤 확정한다.
7. Typer CLI parity를 먼저 완성하고 API/worker/Web은 별도 Gate에서 진행한다.
