...
provider-neutral OIDC/JWKS local signed reference** 단계다. Python Library, local pipeline,
...
1. Product/Security/Platform Owner가 identity/upload decision packet의 실제 issuer, claim, key rotation,
   revocation, assignment와 immutable object 정책을 승인한다.
2. `WP-00.Q1`에서 GOAL 전체 추적, 공개 API 단순성, 모든 script와 정상·오류·경계 example matrix를
   감사하고 local Library 표준화 checkpoint를 만든다.
3. 승인된 값으로 remote discovery/JWKS rotation과 immutable staging adapter를 구현한다.
...
7. Product/Evaluation Owner가 실제 rubric, context→profile mapping과 합격선을 승인한다.
