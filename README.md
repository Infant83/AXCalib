...
provider-neutral OIDC/JWKS local signed reference**, **WP-06.I5a `axcalib verify qwen` local
interface**, **WP-00.Q1 Case/readability 표준화 local Alpha**
...
- strict JSONL batch는 manifest hash와 item별 checkpoint/부분실패를 보존하며 기본 CLI concurrency는
  1로 안전하게 시작한다.
- optional `cli` extra의 Typer/Rich CLI가 pipeline list/run, run status/cancel, batch, maintenance와
  provider-independent `verify qwen`을 Library 구현에 직접 위임한다.
...
| [WP-06.I5a Qwen Verification CLI 리포트](docs/evaluation/wp06-i5a-qwen-verification-cli-report.md) | shared Library probe, `axcalib verify qwen`, portable on-prem runbook과 코드리뷰 |
...
인증 결과가 아니다. 정상·오류·경계 EX-01~EX-14 전체 명령은
...
uv run --no-sync axcalib verify qwen `
  ...
  --scope deployment --output output/onprem/qwen35-capability.json
~~~

`workspace maintain`은 기본 report-only다. `--apply`를 명시해도 삭제하지 않고 quarantine/archive한다.
`verify qwen`은 canonical `OPENAI_*`를 요구하고 합성 text/image만 보내며 secret-free report를
만든다. 나머지 전체 `dossier/evaluate/cases/report` 명령 트리는 G4에서 계속 확장한다.
...
2. `WP-00.Q1`의 Case read facade, GOAL/script trace와 EX-01~14 local 표준화 checkpoint를 회귀
...
4. 첫 `verify qwen` 명령을 기준으로 Alpha CLI를 목표
   `dossier/evaluate/cases/batch/report/verify` 명령 UX로 점진 확장한다.
