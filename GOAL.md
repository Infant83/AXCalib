...
2026-07-24 WP-06.I5a에서는 기존 Qwen capability 구현을 script와 CLI가 공유하는 Library service로
정리하고 `axcalib verify qwen`을 추가했다. canonical `OPENAI_*`, exact checkpoint, synthetic
text/vision, secret-free report와 exit code 의미를 유지하며 사내 Wiki runbook으로 배포한다. 이
interface는 exact on-prem 실행을 준비할 뿐 실제 endpoint나 심사 품질을 대신 검증하지 않는다.
...
- `Case` live read facade, readable actual-PPT lifecycle example와 EX-01~EX-14 self-check catalog
...
- 같은 Library probe를 호출하는 `axcalib verify qwen` operator interface
...
현재 local Alpha evidence: sync/async executor, bounded JSONL batch, `verify qwen`을 포함한 Alpha CLI,
fail-closed FastAPI
...
axcalib verify qwen --scope deployment --output .\qwen-capability.json
~~~
...
- [x] script와 같은 Library service를 사용하는 `axcalib verify qwen`과 portable on-prem runbook
...
- [x] project-id-bound Case status/summary facade, readable actual-PPT lifecycle와 EX-01~EX-14 audit catalog
