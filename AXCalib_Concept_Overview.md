...
이 흐름은 고정된 파이프라인을 의미하지 않는다. 실제 구현에서는 일부 단계를 병렬로 수행하거나, 여러 차례 반복하거나, 사람의 검토를 삽입하거나, 외부 시스템에 위임할 수 있다.
...
Agent가 평가를 자동화하더라도 고위험 또는 낮은 확신의 결과에는 사람의 검토를 삽입할 수 있어야 한다.
...
| 초기 구현 형태 | 미정; 라이브러리 또는 소규모 평가 Agent부터 시작 가능 |
