...
> **근거를 정렬하고 판단을 보정하여, 권한 있는 사람의 AX 인증 결정을 돕는 에이전트 라이브러리**
...
AXCalib는 라이브러리, CLI, API, 서비스, Web App 또는 평가 하네스로 발전할 수 있다. 이 문서는
명명과 장기 철학을 설명하고, 현재 구현 계약은 WORK_SPEC.md, GOAL.md, DESIGN.md와 승인된 ADR이
고정한다.
...
**AX 역량 평가와 보정을 통해 권한 있는 사람의 인증 결정을 지원하는 에이전트 라이브러리**

### 2.4 한 줄 정의

> AXCalib는 다양한 평가 증거를 구조화하고 판단 편차를 보정하여, 권한 있는 사람이 AX 역량
> 수준과 인증을 근거 중심으로 결정하도록 지원하는 확장 가능한 에이전트 라이브러리다.

### 2.5 영문 한 줄 정의

> AXCalib is an extensible agent library that structures evidence and calibrates assessment so authorized humans can make traceable AX certification decisions.
...
동시에 `Calib`는 **Calibration**을 자연스럽게 연상시킨다. 이는 AXCalib가 단순히 답안을 채점하는
도구가 아니라, 평가 기준과 에이전트 판단을 지속적으로 보정하고 사람이 신뢰할 수 있는 인증
결정을 내리도록 돕는 체계를 지향한다는 점과 잘 맞는다.
...
Excalibur의 “아무나 뽑을 수 없는 칼” 이미지는 AXCalib 철학을 쉽게 설명하기 위한 **비유**다.
공식 어원, 상표 선언, 제품명 변경 또는 자동 인증 알고리즘을 뜻하지 않는다.

> **근거가 자격을 만들고, 보정이 판단을 맞추며, 권한 있는 사람이 인증한다.**
>
> Evidence qualifies. Calibration aligns. Authorized humans certify.

| Excalibur 비유 | AXCalib 의미 |
|---|---|
| 돌과 칼 | 승인된 rubric과 정책으로 보호된 인증 권한 |
| 모여드는 조각 | dossier의 과제 증거와 source locator |
| 정렬되는 고리 | 모델·기준·평가자 편차를 점검하는 calibration |
| 조력자 | 근거와 불확실성을 정리해 제안 리포트를 만드는 Agent |
| 칼을 뽑는 권한자 | 관리자 HITL 뒤 최종 상태를 확정하는 사람 |
| 남겨진 칼집과 기록 | snapshot, notification, reviewer decision, audit |

![AXCalib 권한 중심 철학](docs/manuals/assets/axcalib-authority-hero.jpg)

Agent는 칼을 뽑지 않는다. 근거가 부족하면 채우지 않고, 유사사례를 정답으로 사용하지 않으며,
승인요청 알림과 사람 검토 없이 인증 상태를 확정하지 않는다.
...
- **Agentic**: 에이전트가 증거 수집, 분석, 채점, 검토, 제안 및 설명을 협력적으로 수행하고 사람 결정에서 대기한다.
...
| Leveling Agent | 여러 평가 결과를 종합하여 AX Level 제안과 불확실성을 생성 |
| Certification Agent | 인증 전제조건을 검토하고 사람 검토용 인증 가능성 제안을 생성 |
...
다음 코드는 ADR-014가 고정한 **pre-implementation public contract**다. WP-01 구현과 test가
완료되기 전에는 실행 가능한 제품 API라고 표시하지 않는다.

```python
from axcalib import AXCalib

client = AXCalib()  # offline-safe defaults
result = client.evaluate(
    "AXC-demo-001.axc.yaml",
    stage="registration",
)

print(result.status)     # succeeded | waiting_human | blocked | ...
print(result.report_id)  # Agent proposal report reference
```

비동기는 같은 의미의 `await client.aevaluate(...)`를 사용한다. model, retrieval, storage 또는
notification adapter가 필요한 전문 사용자는 다음처럼 별도 profile을 연다.

```python
client = AXCalib.from_toml(
    "config/axcalib.expert.toml",
    profile="onprem",
)
```

CLI와 API는 같은 application service를 호출한다.

```bash
axcalib dossier validate AXC-demo-001.axc.yaml
axcalib evaluate registration AXC-demo-001.axc.yaml
axcalib batch status RUN-001
axcalib verify run RUN-001
```

요청별 제어는 OpenAPI 3.1의 typed `options` JSON만 허용한다. `auto_certify`, `skip_hitl` 또는
임의 Python import path는 TOML과 API에 제공하지 않는다.
...
> **Calibrate Assessment. Support Trusted AX Decisions.**
...
> AXCalib는 사람의 근거 중심 AX 인증 결정을 돕는 에이전트 라이브러리다.

### 중간 길이 설명

> AXCalib는 수행 증거를 구조화하고 평가 편차를 보정하여, 권한 있는 사람이 AX Level과 인증을
> 추적 가능한 근거로 결정하도록 지원하는 확장 가능한 에이전트 라이브러리다.

### 기술 중심 설명

> AXCalib provides composable evidence pipelines, calibrated assessment workflows, and auditable
> certification primitives while keeping final authority with designated humans.
...
| 권한 원칙 | Agent는 제안하고 승인된 사람이 최종결정 |
| 기억 문장 | Evidence qualifies. Calibration aligns. Authorized humans certify. |
| 사용 경험 | 최소 facade에서 TOML profile/OpenAPI JSON으로 점진 확장 |
...
| 2026-07-15 | Excalibur 기억 장치, 사람 최종권한, 최소 facade, progressive config/OpenAPI 사용철학 반영 |
