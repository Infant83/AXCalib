...
baseline: v0.3
harness_status: executable_offline_harness_built; domain_mvp_not_built
...
| G1 Harness | package scaffold, prep, state/decision/risk | 구현 완료, status/validate read-only 검증 후 owner review |
...
`prep.ps1`과 executable offline harness는 구현되어 있다. 현재 eval은 synthetic workflow
contract만 검증하며 parser, model 또는 retrieval 품질을 검증하지 않는다.
...
| 18. 요소 모듈·국소 pipeline·전체 workflow 조합 | FR-033~036, DESIGN 4, ADR-013, architecture plan | Architecture contract specified; implementation pending |
...
- [ ] PipelineContext/Result/Registry와 첫 working pipeline script
